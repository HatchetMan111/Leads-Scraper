#!/usr/bin/env bash
# =============================================================================
# LeadScraper + Sales Team — Proxmox LXC Installer (Community-Scripts-Stil)
#
...
#   3. Schiebt App-Dateien (app/, icp/, systemd/, setup) in den Container
#   4. Installiert dort LeadScraper + Web UI als systemd-Service
#   5. Verifiziert Service + HTTP und gibt die finale URL aus
#
# Belegte CT-ID -> automatisch nächste freie nehmen (LXC *und* VM geprüft).
# Debugging:  DEBUG=1 bash -x install/leadscraper.sh   (volles Trace-Log)
# =============================================================================
set -euo pipefail

# ============================ VARIABLEN (oben) ================================
APP="leadscraper"
SERVICE="leadscraper.service"
GITHUB_USER="${GITHUB_USER:-HatchetMan111}"
GITHUB_REPO="${GITHUB_REPO:-Leads-Scraper}"
...
DEFAULT_HOSTNAME="${DEFAULT_HOSTNAME:-leadscraper}"
...
CTID=$((10#${CTID}))  # 10er-Basis (führende Nullen nicht als Oktal)
if [[ "${CTID}" -lt 100 ]]; then
  echo "Ungültige CT-ID: ${CTID} (Proxmox-IDs beginnen bei 100)." >&2; exit 1
fi

id_taken() { # 0 = belegt — prüft LXC *und* QEMU-VMs.
  local id=$1
...
# --- App-Dateien in den Container schieben ------------------------------------
WORKDIR="$(mktemp -d)"
cleanup() { rm -rf "${WORKDIR}"; }
trap 'cleanup; fail' ERR
echo "-> Lade Repo (${GITHUB_USER}/${GITHUB_REPO}@${GITHUB_BRANCH}) ..."
...
# pct push kann nur DATEIEN -> alles als Tarball pushen, im CT entpacken.
tar -czf "${WORKDIR}/wrapper.tar.gz" -C "${WORKDIR}/repo" app icp systemd install/setup-container.sh
pct exec "${CTID}" -- mkdir -p /opt/leadscraper
pct push "${CTID}" "${WORKDIR}/wrapper.tar.gz" /opt/leadscraper/wrapper.tar.gz
pct exec "${CTID}" -- test -f /opt/leadscraper/wrapper.tar.gz \
  || { echo "Tarball-Push in CT:${CTID} fehlgeschlagen." >&2; exit 1; }
pct exec "${CTID}" -- tar -xzf /opt/leadscraper/wrapper.tar.gz -C /opt/leadscraper
pct exec "${CTID}" -- chmod +x /opt/leadscraper/install/setup-container.sh
pct exec "${CTID}" -- test -f /opt/leadscraper/app/app.py \
  || { echo "Tarball unvollständig im Container angekommen." >&2; exit 1; }

# --- Setup IM Container ausführen ----------------------------------------------
echo "-> Führe Setup im Container aus ..."
pct exec "${CTID}" -- env WEB_PORT="${WEB_PORT}" DEBUG="${DEBUG:-0}" \
  bash /opt/leadscraper/install/setup-container.sh

# --- Verifikation vom Host -----------------------------------------------------
echo "-> Verifikation ..."
pct exec "${CTID}" -- systemctl is-active --quiet leadscraper \
  || { echo "Service läuft NICHT. Log:" >&2
       pct exec "${CTID}" -- journalctl -u leadscraper --no-pager -n 100 >&2
       exit 1; }
...
echo "=================================================================="
echo " ✅ Fertig! LeadScraper Web UI: http://${CT_IP}:${WEB_PORT}"
echo "    CT-ID ${CTID} (${HOSTNAME}), onboot=1, Service=leadscraper"
echo "    Update : pct exec ${CTID} -- bash /opt/leadscraper/install/setup-container.sh"
echo "    Logs   : pct exec ${CTID} -- journalctl -u leadscraper -f"
